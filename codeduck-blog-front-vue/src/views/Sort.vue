      ...
      <el-col :xs="4" :sm="4" :md="4" :lg="4" :xl="2">
        <div class="sidebar">
          <el-timeline :reverse="reverse">
            <el-timeline-item v-for="(item, index) in sortList" hide-timestamp :key="index">
              <span
...
export default {
  ...
  methods: {
    /* 获取分类列表信息 */
    getSortList () {
      SortApi.getSortList().then(res => {
        if (res.code !== 1000) {
          return this.$message.error('系统开小差了……')
        }
        this.sortList = res.data
        this.getArticles(this.sortList[0].id)
      })
    },
    /* 根据分类Id获取博文列表 */
    getArticles (sortId) {
      this.selectedId = sortId
      SortApi.getArticlesBySortId(sortId).then(res => {
        if (res.code !== 1000) {
          return this.$message.error('系统开小差了……')
        }
